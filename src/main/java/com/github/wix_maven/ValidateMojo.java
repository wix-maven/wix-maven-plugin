...
public class ValidateMojo extends AbstractPackageable {
  ...
  public void execute() throws MojoExecutionException {
    ...
    // considering some artifact types, like wixlib, as folders so that link step (lit) doesn't need
    // to be run to test later compile steps... Run it as part of packaging.
    //
    // File archOutputFile;
    // for (String arch : getPlatforms()) {
    //
    // String baseCulturespec = baseCulturespec();
    // String baseClassifier = arch + "-" + (baseCulturespec == null ? "neutral" :
    // getPrimaryCulture(baseCulturespec));
    //
    // if( ! ML_REPACK.equalsIgnoreCase(mergeLevel) || packLevel.contains(PL_PACKAGE) ){ //
    // none,trans need to include base, full include every
    ...
    // && ( PACK_INSTALL.equalsIgnoreCase(getPackaging()) ||
    ...
    // || ML_TRANSFORM.equalsIgnoreCase(mergeLevel) && ( packLevel.contains(PL_DEFAULT) ||
    // packLevel.contains(PL_MST))
    // ) {
