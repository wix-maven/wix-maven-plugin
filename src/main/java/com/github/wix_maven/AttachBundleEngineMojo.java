...
/**
 * Attach the (signed) bundle engine back to the bundle. TODO: might be more appropriate to use
 * custom phase. insignia -ab engine.exe bundle.exe -o bundle.exe ... sign bundle.exe
 */
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    for (String arch : getPlatforms()) {
      for (String culture : culturespecs()) {
        ...
        File resovledBundleEngineFile =
            new File(getOutputPath(bundleEnginePath, arch, culture), bundleEngineName);

        cl.addArguments(new String[] {"-ab", resovledBundleEngineFile.getAbsolutePath(),
            archOutputFile.getAbsolutePath(), "-out", archOutputFile.getAbsolutePath()});
