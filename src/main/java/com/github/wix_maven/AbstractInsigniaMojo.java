...
public abstract class AbstractInsigniaMojo extends AbstractPackageable {
  ...
  /**
   * TODO: make this a pattern so no need for seperate filename The file path for detach/attach of
   * the bundle engine. - $(arch) ${culture}
   */
  @Parameter(property = "wix.bundleEnginePath",
      defaultValue = "${project.build.directory}/wix-bundleEngines", required = true)
  protected File bundleEnginePath;

  /**
   * TODO: merge with above once it is a pattern so no need for seperate filename The file name for
   * detach/attach of the bundle engine.
   */
