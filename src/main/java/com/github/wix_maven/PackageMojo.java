...
public class PackageMojo extends AbstractPackageable {
  ...
  // TODO: work out how we should package - many different parts now, should the be allowed
  // separately
  public final void execute() throws MojoExecutionException, MojoFailureException {
    ...
    for (String arch : getPlatforms()) {
      ...
      String baseClassifier =
          arch + "-" + (baseCulturespec == null ? "neutral" : getPrimaryCulture(baseCulturespec));

      if (!ML_REPACK.equalsIgnoreCase(mergeLevel) || packLevel.contains(PL_PACKAGE)) { // none,trans
                                                                                       // need to
                                                                                       // include
                                                                                       // base, full
                                                                                       // include
      ...
      if (!ML_BASE.equalsIgnoreCase(mergeLevel)
          && (PACK_INSTALL.equalsIgnoreCase(getPackaging()) || PACK_PATCH
        ...
        for (String culturespec : alternateCulturespecs()) {
          ...
          if (ML_REPACK.equalsIgnoreCase(mergeLevel) && packLevel.contains(PL_MST)
              || ML_TRANSFORM.equalsIgnoreCase(mergeLevel)
              && (packLevel.contains(PL_DEFAULT) || packLevel.contains(PL_MST))) {
            ...
            if (archMSTOutputFile.exists())
              ...
              getLog().warn(
                  String.format("Missing expected mst file for architecture", archMSTOutputFile,
                      culturespec));
          }
          ...
          if (packLevel.contains(PL_PACKAGE)
            ...
            if (archOutputFile.exists())
              ...
              getLog().warn(
                  String.format("Missing expected msi file for architecture", getPackaging(),
                      archOutputFile, culturespec));
          }
        }
      }
    }
  ...
  private void packageCabs(String[] cabs, String arch, File sourceDir, String classifierBase,
      File cabsCollection, String culturespec) {
    ...
    for (String cab : cabs) {
      ...
      if (!archCabFile.exists() || !archCabFile.isFile())
        ...
        // projectHelper.attachArtifact( project, "cab", classifierBase + "-" + archCabFile,
        // archCabFile );
      }
    }
  }
