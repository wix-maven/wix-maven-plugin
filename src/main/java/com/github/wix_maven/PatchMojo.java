...
//		filter.addFilter(new ProjectTransitivityFilter(project.getDependencyArtifacts(), true));
	...
	private void addJARSourceRoots(Commandline cl, Artifact inputArtifact, String bindPathOpt ) throws MojoExecutionException {
...
//			Map<String,File> archIntermediateFiles = new HashMap<String, File>();
			for (String culture : culturespecs()) {
				...
				Artifact baseInputArtifact = getRelatedArtifact( baseArtifactItem, arch, culture, baseArtifactItem.getType());
				Artifact latestInputArtifact = getRelatedArtifact( patchedArtifactItem, arch, culture, patchedArtifactItem.getType());
...
//			pyro(pyroTool, patchFile, archIntermediateFiles, outputFile);
	 ...
	 */
	protected Artifact getRelatedArtifact(ArtifactItem artifactItem, String arch, String culture, String type) throws MojoExecutionException {
...
//	);
    ...
    {
        Set<Artifact> artifacts = theProject.createArtifacts( this.factory, null, new ScopeArtifactFilter( Artifact.SCOPE_COMPILE ) );
// doubt: scope if not null is ignoring the Provided scope elements - all examples show using some scope  
        ...
        InvalidDependencyVersionException
    {
        Artifact pomArtifact =
            this.factory.createArtifact( artifact.getGroupId(), artifact.getArtifactId(), artifact.getVersion(), "", "pom" );

        MavenProject pomProject = mavenProjectBuilder.buildFromRepository( pomArtifact, this.remoteArtifactRepositories, this.localRepository );
